defmodule Funnel.Investigator.HelpersTest do
  ...
  test "fail_open_pull_requests/2 fails all associated open pull requests" do
    with_mocks([
      ...
      use_cassette "default_branch_pushed" do
        Funnel.Investigator.Helpers.fail_open_pull_requests(
          build(:failure_status).description,
          build(:push_scent),
          @tenta_client
        )
  ...
  test "get_base_sha/2 returns a sha" do
    with_mocks([
      {Tentacat.Pulls, [],
       [filter: fn _, _, _, _ -> {200, [%{"base" => %{"ref" => "branch"}}], nil} end]},
      {Tentacat.Repositories.Branches, [],
       [find: fn _, _, _, _ -> {200, %{"commit" => %{"sha" => "sha1"}}, nil} end]}
    ]) do
